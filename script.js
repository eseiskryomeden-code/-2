...
// password policy check: 최소 4자, 영어(letter) 1개 이상, 특수문자 1개 이상
function passwordValid(pw){
  if(typeof pw !== 'string') return false;
  if(pw.length < 4) return false;
  if(!/[A-Za-z]/.test(pw)) return false; // 영문자
  if(!/[^A-Za-z0-9]/.test(pw)) return false; // 특수문자
  return true;
}
...
const signupRoleEl = document.getElementById('signup-role');
const signupBtn = document.getElementById('signup-btn');
const signupTeacherCodeEl = document.getElementById('signup-teacher-code'); // may be null if not added to HTML
...
addAnnBtn && addAnnBtn.addEventListener('click', ()=>{
  const title = prompt('공지 제목을 입력하세요:'); if(!title) return;
  const text = prompt('공지 내용을 입력하세요:'); if(text === null) return;
  const list = loadAnns(); list.push({title:title.trim(), text:text.trim(), time: new Date().toLocaleString()});
  saveAnns(list); renderAnns();
});
...
if(surveyForm){
  surveyForm.addEventListener('submit', e=>{
    ...
    const rating = document.getElementById('survey-rating').value;
    const comment = document.getElementById('survey-comment').value.trim();
    if(!rating){ alert('만족도를 선택하세요.'); return; }
    const arr = loadSurvey(); arr.push({name:name||'익명', rating:rating, comment:comment, time:new Date().toLocaleString()});
    saveSurvey(arr); updateSurveySummary(); surveyForm.reset(); alert('설문 제출 완료');
  });
}
...
function renderGrades(){
  ...
  arr.slice().reverse().forEach((g, idxRev)=>{
    const idx = arr.length-1-idxRev;
    const item = document.createElement('div'); item.className='grade-item';
    item.innerHTML = `<h4>${escapeHtml(g.title)} <small style="font-weight:600;color:#6b7280">${escapeHtml(g.subject)}</small></h4><div>${escapeHtml(g.desc)}</div><div class="hint">제출기한: ${g.due||'없음'} | 작성자: ${escapeHtml(g.author||'관리자')} | ${g.time}</div>`;
    const s = getSession(); const isTeacher = s && s.role==='teacher'; const isAuthor = s && s.username===g.author;
    if(isTeacher || isAuthor){
      const del = document.createElement('button'); del.textContent='삭제'; del.style.marginTop='6px';
      del.addEventListener('click', ()=>{ if(confirm('삭제하시겠습니까?')){ deleteGrade(idx); } });
      item.appendChild(del);
...
if(gradeForm){
  gradeForm.addEventListener('submit', e=>{
    e.preventDefault();
    const s = getSession();
    if(!s || s.role !== 'teacher'){ alert('교사 계정으로 로그인해야 작성할 수 있습니다.'); return; }
    const title = document.getElementById('grade-title').value.trim();
    const subject = document.getElementById('grade-subject').value.trim();
    const desc = document.getElementById('grade-desc').value.trim();
    const due = document.getElementById('grade-due').value || null;
    addGrade({ title, subject, desc, due, author: s.username, time: new Date().toLocaleString() });
    gradeForm.reset();
  });
}
...
// show/hide teacher code input on role change
if(signupRoleEl){
  signupRoleEl.addEventListener('change', ()=>{
    const wrap = document.getElementById('signup-teacher-code-wrap');
    if(wrap) wrap.style.display = (signupRoleEl.value === 'teacher') ? 'block' : 'none';
  });
}
...
if(signupBtn){
  signupBtn.addEventListener('click', async ()=>{
    try{
      ...
      const role = signupRoleEl.value;
      if(pw !== pw2) { alert('비밀번호가 일치하지 않습니다.'); return; }
      // fetch teacher code from the input (if present) or null
      const teacherCode = (signupTeacherCodeEl && signupTeacherCodeEl.value) ? signupTeacherCodeEl.value.trim() : null;
